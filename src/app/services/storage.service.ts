...
export class DatabaseService {
  ...
  getAnswers(questionId): Observable<Answer[]> {
  ...
  getAnswersByQuestionId(questionId) {
    ...
    return this.database.executeSql('SELECT answer.id, answer.title, answer.isCorrect, answer.questionId FROM answer JOIN question ON question.id = answer.questionId JOIN quiz ON quiz.id = question.quizId WHERE question.id = ?', [questionId]).then(data => {
